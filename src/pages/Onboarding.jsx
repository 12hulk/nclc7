...
const QUESTIONS = {
  ...
  A2: [
    { skill: "Comprehension", q: "Read this text and answer.", ctx: "Je m'appelle Fatima. Je viens du Maroc. J'habite à Montréal depuis six mois. Je travaille dans un café le matin.", opts: ["She works in a café", "She is a student", "She works in an office", "She does not work"], c: 0, ex: "Je travaille dans un café = I work in a café. Travailler = to work. Depuis = since/for." },
    ...
    { skill: "Practical", q: "You call a clinic and hear this. What should you do?", ctx: "Nos bureaux sont fermés. Veuillez rappeler pendant les heures d'ouverture : lundi au vendredi, 8h à 17h.", opts: ["Call back weekday between 8am and 5pm", "Leave a message now", "Go to the clinic immediately", "Call a different number"], c: 0, ex: "Fermés = closed. Rappeler = call back. Heures d'ouverture = opening hours. Critical for navigating services in French Canada." },
    ...
    { skill: "Grammar", q: "Choose the correct form.", ctx: "Nous _______ au Canada depuis deux ans.", opts: ["sommes", "avons", "êtes", "sont"], c: 0, ex: "Nous sommes = We are. Être conjugated for nous = sommes. The most common way to express duration in French." },
    { skill: "Practical", q: "You receive this letter from the city. What is it about?", ctx: "Objet : Renouvellement de votre permis de stationnement\nVotre permis expire le 31 décembre. Veuillez renouveler avant cette date.", opts: ["Parking permit expires Dec 31", "You have a parking fine to pay", "Your parking is cancelled", "You need a new address"], c: 0, ex: "Renouvellement = renewal. Permis = permit. Expire = expires. Reading official mail is a key immigration skill." },
  ],
  B1: [
    { skill: "Grammar", q: "Choose the sentence that uses the correct tense for a completed action yesterday.", ctx: "Context: describing what happened at work yesterday.", opts: ["Hier, j'ai présenté mon rapport à l'équipe.", "Hier, je présente mon rapport à l'équipe.", "Hier, je présentais mon rapport à l'équipe.", "Hier, je vais présenter mon rapport."], c: 0, ex: "For a completed action in the past, use passé composé: j'ai présenté. Présente (present tense) and présentais (imperfect, ongoing) do not fit a completed yesterday action." },
...
.ob-top{display:flex;justify-content:space-between;align-items:center;padding:14px 20px;border-bottom:1px solid #f3ece0;background:rgba(250,246,239,.9);}
.ob-logo{font-family:'Cormorant Garamond',serif;font-size:20px;font-weight:700;color:#0c1420;cursor:pointer;}
.ob-logo span{color:#b8893a;}
.ob-sc{font-size:11px;color:#7a8499;font-weight:500;}
.ob-home{font-size:12px;color:#7a8499;background:transparent;border:1px solid #e8dfd0;padding:5px 12px;border-radius:6px;cursor:pointer;font-family:'Outfit',sans-serif;transition:all .2s;}
.ob-home:hover{border-color:#0c1420;color:#0c1420;}
...
.ob-ctx{background:#faf6ef;border:1px solid #e8dfd0;border-radius:10px;padding:14px 16px;margin-bottom:14px;font-size:14px;color:#0c1420;line-height:1.6;font-weight:500;}
...
export default function Onboarding({ onComplete, onBack }) {
  ...
  const placed = getPlacedLevel(selfLevel, score);
  ...
  const topicLabels = TOPICS.filter((t) => topics.includes(t.id)).map((t) => t.label);
  ...
  const fill =
    ...
    : phase === "target" ? 32
    : phase === "briefing" ? 42
    : phase === "quiz" ? 42 + Math.round((qIdx / (qBank.length || 1)) * 50)
    : 100;
      ...
      <div className="ob-root">
        <div className="ob-card">
          ...
          <div className="ob-top">
            <div className="ob-logo" onClick={goHome}>NCLC<span>7</span></div>
            <div className="ob-sc">{scLabel}</div>
            {phase !== "result" && (
              <button className="ob-home" onClick={goHome}>← Home</button>
            )}
          </div>
          ...
          {phase === "intro" && (
            <div className="ob-wrap">
              ...
              <div className="ob-sub">A practical, adaptive assessment built for immigrants — not a grammar quiz for students. Every question reflects real life in Canada. Takes 8–10 minutes.</div>
              {[
                ...
                ["NCLC placement", "Official Canadian scale score — instantly"],
              ].map(([t, d]) => (
              ...
              <button className="ob-btn" style={{ marginTop: 16, width: "100%" }} onClick={() => setPhase("level")}>
                Begin assessment →
              </button>
            </div>
          )}
          ...
          {phase === "level" && (
            <div className="ob-wrap">
              ...
              <div className="ob-sub">Be honest — the assessment will confirm and may adjust your placement. Your questions will be matched to your level.</div>
              {LEVELS.map((l) => (
              ...
              <div className="ob-btnrow" style={{ marginTop: 8 }}>
                <button className="ob-btnb" onClick={() => setPhase("intro")}>← Back</button>
                <button className="ob-btn" disabled={!selfLevel} onClick={() => setPhase("topics")}>Continue →</button>
              </div>
            </div>
          )}
          ...
          {phase === "topics" && (
            <div className="ob-wrap">
              ...
              <div className="ob-sub">We tailor your practice exercises to the French you actually need day-to-day in Canada.</div>
          ...
          {phase === "target" && (
            <div className="ob-wrap">
              ...
              <div style={{ background: "#faf6ef", border: "1px solid #e8dfd0", borderRadius: 10, padding: "13px 16px", marginBottom: 18, fontSize: 12, color: "#7a8499", lineHeight: 1.7 }}>
                ...
                NCLC 5–6 → Family sponsorship · NCLC 7 → Express Entry (CLB 7) · NCLC 9 → Federal Skilled Worker
              </div>
          ...
          {phase === "briefing" && (
            <div className="ob-wrap">
              ...
              <div className="ob-sub">
                {{ A1: "Survival French — real signs, forms and everyday situations.", A2: "Practical French — reading messages, short texts and daily tasks.", B1: "Working French — grammar, workplace and formal communication.", B2: "Advanced French — complex grammar, formal writing and legal comprehension." }[selfLevel]}
              </div>
              {[
              ...
              <div className="ob-btnrow" style={{ marginTop: 16 }}>
                <button className="ob-btnb" onClick={() => setPhase("target")}>← Back</button>
                <button className="ob-btn" onClick={() => setPhase("quiz")}>Start now →</button>
              </div>
            </div>
          )}
          ...
          {phase === "quiz" && (() => {
            ...
            return (
              <div className="ob-wrap">
                ...
                <div className="ob-h" style={{ fontSize: 18, marginBottom: 12 }}>{q.q}</div>
                {q.ctx && (
                ...
                {answered && showExplain && (
                  <div className="ob-explain">
                    <strong style={{ color: "#b8893a" }}>Why: </strong>{q.ex}
                  </div>
                )}
          ...
          {phase === "result" && (() => {
            ...
            const topicText = topicLabels.length > 0 ? topicLabels.slice(0, 2).join(" & ") : "immigration context";
            return (
              <>
                ...
                <div className="ob-rsub">
                  {score >= 75
                    ? "Strong performance — your actual level may be higher than you thought."
                    : score >= 50
                    ? "Solid foundation. Targeted practice will close the gap quickly."
                    : "Good starting point — focused daily practice makes the difference."}
                </div>
                    ...
                    {skillKeys.map((sk) => {
                      ...
                      return (
                        <div key={sk} className="ob-skillbox">
                          ...
                          <div className="ob-skillbar">
                            <div className="ob-skillfill" style={{ width: `${pct}%` }} />
                          </div>
                ...
                <div className="ob-pbox">
                  ...
                  {[
                    { icon: "✍️", text: `Writing · 3x/week · focus on ${weakSkills.length > 0 ? weakSkills.join(", ") : "all skills"}` },
                    { icon: "🎧", text: "Listening · 2x/week · Québec accent audio at your level" },
                    ...
                    { icon: "📚", text: `Vocabulary · 10 words/day · ${topicText}` },
                  ].map((item, i) => (
